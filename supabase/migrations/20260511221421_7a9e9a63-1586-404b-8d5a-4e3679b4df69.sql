...
CREATE POLICY "participants_view_own_edit_requests" ON public.profile_edit_requests FOR SELECT USING (requester_id = auth.uid());
DROP POLICY IF EXISTS "staff_manage_edit_requests" ON public.profile_edit_requests;
CREATE POLICY "staff_manage_edit_requests" ON public.profile_edit_requests FOR ALL
  USING (has_any_role(auth.uid(), ARRAY['executive','assistant','board','project_manager','branch_manager']::app_role[]) OR is_system_admin(auth.uid()));
DROP POLICY IF EXISTS "participants_create_edit_requests" ON public.profile_edit_requests;
CREATE POLICY "participants_create_edit_requests" ON public.profile_edit_requests FOR INSERT WITH CHECK (requester_id = auth.uid());
...
CREATE POLICY "staff_manage_memberships" ON public.participant_project_memberships FOR ALL
  USING (has_any_role(auth.uid(), ARRAY['executive','assistant','board','project_manager','branch_manager','employee']::app_role[]) OR is_system_admin(auth.uid()));
DROP POLICY IF EXISTS "participants_view_own_memberships" ON public.participant_project_memberships;
CREATE POLICY "participants_view_own_memberships" ON public.participant_project_memberships FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.participants p WHERE p.id = participant_id AND p.auth_user_id = auth.uid()));
...
CREATE TABLE IF NOT EXISTS public.message_threads (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  subject TEXT,
  thread_type TEXT NOT NULL DEFAULT 'direct' CHECK (thread_type IN ('direct','project','broadcast')),
...
CREATE POLICY "thread_members_access" ON public.message_threads FOR ALL
  USING (created_by = auth.uid()
    OR EXISTS (SELECT 1 FROM public.message_thread_members WHERE thread_id = id AND user_id = auth.uid())
    OR is_system_admin(auth.uid())
    OR has_any_role(auth.uid(), ARRAY['executive','assistant']::app_role[]));
DROP POLICY IF EXISTS "thread_member_access" ON public.message_thread_members;
CREATE POLICY "thread_member_access" ON public.message_thread_members FOR ALL
  USING (user_id = auth.uid() OR is_system_admin(auth.uid()) OR has_role(auth.uid(),'executive'::app_role));
DROP POLICY IF EXISTS "messages_access" ON public.messages;
CREATE POLICY "messages_access" ON public.messages FOR ALL
  USING (sender_id = auth.uid()
    OR EXISTS (SELECT 1 FROM public.message_thread_members WHERE thread_id = messages.thread_id AND user_id = auth.uid())
    OR is_system_admin(auth.uid())
...
CREATE TABLE IF NOT EXISTS public.support_tickets (
  ...
  title TEXT NOT NULL,
  body TEXT NOT NULL,
  category TEXT NOT NULL DEFAULT 'general' CHECK (category IN ('general','technical','billing','account','content','other')),
...
CREATE POLICY "users_manage_own_tickets" ON public.support_tickets FOR ALL USING (requester_id = auth.uid()) WITH CHECK (requester_id = auth.uid());
DROP POLICY IF EXISTS "staff_manage_all_tickets" ON public.support_tickets;
CREATE POLICY "staff_manage_all_tickets" ON public.support_tickets FOR ALL
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','employee']::app_role[]));
DROP POLICY IF EXISTS "users_view_own_ticket_replies" ON public.support_ticket_replies;
CREATE POLICY "users_view_own_ticket_replies" ON public.support_ticket_replies FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.support_tickets WHERE id = ticket_id AND requester_id = auth.uid()) AND is_internal = false);
DROP POLICY IF EXISTS "staff_manage_ticket_replies" ON public.support_ticket_replies;
CREATE POLICY "staff_manage_ticket_replies" ON public.support_ticket_replies FOR ALL
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','employee']::app_role[]));
...
CREATE TABLE IF NOT EXISTS public.lms_activities (
  ...
  course_id UUID NOT NULL REFERENCES public.lms_courses(id) ON DELETE CASCADE,
  activity_type TEXT NOT NULL DEFAULT 'assignment' CHECK (activity_type IN ('assignment','exercise','project','discussion','quiz')),
  title_ar TEXT NOT NULL,
  title_en TEXT,
  description TEXT,
...
CREATE POLICY "lms_modules_published_visible" ON public.lms_modules FOR SELECT
  USING (is_published = true OR is_system_admin(auth.uid())
    OR has_any_role(auth.uid(), ARRAY['executive','assistant','project_manager','branch_manager','employee']::app_role[]));
DROP POLICY IF EXISTS "staff_manage_lms_modules" ON public.lms_modules;
CREATE POLICY "staff_manage_lms_modules" ON public.lms_modules FOR ALL
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','project_manager','branch_manager','employee']::app_role[]));

DROP POLICY IF EXISTS "activities_visible_to_enrolled" ON public.lms_activities;
CREATE POLICY "activities_visible_to_enrolled" ON public.lms_activities FOR SELECT USING (true);
DROP POLICY IF EXISTS "staff_manage_activities" ON public.lms_activities;
CREATE POLICY "staff_manage_activities" ON public.lms_activities FOR ALL
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','project_manager','branch_manager','employee']::app_role[]));

DROP POLICY IF EXISTS "users_own_submissions" ON public.lms_activity_submissions;
CREATE POLICY "users_own_submissions" ON public.lms_activity_submissions FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "staff_view_submissions" ON public.lms_activity_submissions;
CREATE POLICY "staff_view_submissions" ON public.lms_activity_submissions FOR SELECT
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','project_manager','branch_manager','employee']::app_role[]));
DROP POLICY IF EXISTS "staff_grade_submissions" ON public.lms_activity_submissions;
CREATE POLICY "staff_grade_submissions" ON public.lms_activity_submissions FOR UPDATE
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant','project_manager','branch_manager','employee']::app_role[]));

DROP POLICY IF EXISTS "learning_paths_visible" ON public.lms_learning_paths;
CREATE POLICY "learning_paths_visible" ON public.lms_learning_paths FOR SELECT
  USING (is_published = true OR is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant']::app_role[]));
DROP POLICY IF EXISTS "staff_manage_learning_paths" ON public.lms_learning_paths;
CREATE POLICY "staff_manage_learning_paths" ON public.lms_learning_paths FOR ALL
  USING (is_system_admin(auth.uid()) OR has_any_role(auth.uid(), ARRAY['executive','assistant']::app_role[]));

DROP POLICY IF EXISTS "instructor_notes_access" ON public.lms_instructor_notes;
CREATE POLICY "instructor_notes_access" ON public.lms_instructor_notes FOR ALL
  USING (author_id = auth.uid() OR student_id = auth.uid()
    OR is_system_admin(auth.uid()) OR has_role(auth.uid(),'executive'::app_role));
...
-- 9. AUTO MEMBERSHIP TRIGGER
CREATE OR REPLACE FUNCTION public.auto_create_project_membership()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NEW.project_id IS NOT NULL AND (TG_OP = 'INSERT' OR OLD.project_id IS NULL OR OLD.project_id <> NEW.project_id) THEN
    ...
    ON CONFLICT (participant_id, project_id) DO UPDATE SET status = 'active', branch_id = EXCLUDED.branch_id;
  END IF;
  RETURN NEW;
END;
$$;
...
RETURNS INTEGER LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE m.sender_id <> _user_id AND (mtm.last_read_at IS NULL OR m.created_at > mtm.last_read_at) AND m.is_deleted = false;
$$;
CREATE OR REPLACE FUNCTION public.get_unread_notification_count(_user_id UUID)
RETURNS INTEGER LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT COUNT(*)::INTEGER FROM public.notifications WHERE target_user_id = _user_id AND is_read = false;
$$;
